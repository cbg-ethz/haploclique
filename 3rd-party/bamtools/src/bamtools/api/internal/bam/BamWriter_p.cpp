...
void BamWriterPrivate::WriteAlignment(const BamAlignment& al) {
    ...
    assert(al.Qualities.size() == queryLength);
    char baseQualitiesBuffer[queryLength];
    for ( size_t i = 0; i < queryLength; ++i )
        baseQualitiesBuffer[i] = al.Qualities[i] - 33; // FASTQ conversion
    m_stream.Write(baseQualitiesBuffer, queryLength);
